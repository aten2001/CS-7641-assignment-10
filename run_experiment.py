...
def run_experiment(
    experiment_details, experiment, timing_key, dim, skiprerun, verbose, timings
):
    t = datetime.now()
        ...
        if not skiprerun:
            logger.info(
                "Running {} experiment: {} ({})".format(
                    timing_key, details.ds_readable_name, dim
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim",
        type=int,
        help="The dim parameter to use for clustering with a specific experiment "
    ...
    parser.add_argument(
        "--benchmark", action="store_true", help="Run the benchmark experiments"
    )
    ...
    if args.dim or args.skiprerun:
        if (
            ...
            logger.error(
                "Cannot specify dimension/skiprerun without specifying a specific experiment"
            )
            parser.print_help()
            sys.exit(1)
    ...
    if args.skiprerun and not args.dim:
        logger.error(
            "Cannot specify skiprerun without specifying a specific experiment"
        )
        parser.print_help()
        sys.exit(1)
    ...
    dataset1_details = {
        ...
        "readable_name": "poisonous_mushrooms",
        "best_nn_params": {
            "NN__activation": ["relu"],
            "NN__alpha": [1.0],
            "NN__hidden_layer_sizes": [(36, 36)],
